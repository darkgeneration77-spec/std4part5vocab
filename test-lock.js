...
function unlockAfterSubmit(){
 if(!locked)return;
 locked=false;
 sessionStorage.removeItem(key());
 document.querySelectorAll('details').forEach(d=>d.style.display='');
 document.querySelectorAll('.modebtn[data-mode]').forEach(b=>setDisabled(b,false));
 const testBtn=document.querySelector('.modebtn[data-mode="test"]');if(testBtn)testBtn.textContent='Start Test';
 setDisabled(document.getElementById('reshuffle'),false);
 document.querySelectorAll('.listen-btn,.speak').forEach(b=>setDisabled(b,false));
 const wrong=document.querySelector('.wrongbox');if(wrong)wrong.style.display='';
 const game=document.querySelector('.game');if(game)game.style.display='';
 const notice=document.getElementById('testLockNotice');if(notice){notice.textContent='TEST SUBMITTED — Study tools are available again.';setTimeout(()=>notice.remove(),3500)}
}
function init(){
 const btn=document.querySelector('.modebtn[data-mode="test"]');if(!btn)return;
 btn.textContent='Start Test';
 if(sessionStorage.getItem(key())==='1'){lockTest(true)}
 document.addEventListener('click',e=>{
   const t=e.target.closest('.modebtn[data-mode="test"]');if(!t||locked)return;
   e.preventDefault();e.stopImmediatePropagation();
   if(confirm('Start Test now? Once started, Notes, listening, revision and games will stay locked until this test is submitted.'))lockTest(false);
 },true);
 window.addEventListener('wordscoop-test-submitted',e=>{if(e.detail?.mode==='test')unlockAfterSubmit()});
}
